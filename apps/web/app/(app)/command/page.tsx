import { Button, Card, Input, Badge } from '@aetheros/ui'
;('use client')
...
export default function CommandCenter() {
        ...
        if (Array.isArray(meetings)) {
          ...
          if (pending && pending.proposed_slots && pending.proposed_slots.length > 0) {
            ...
            setActiveProposal({
              ...
              attendees: (pending.participants || []).map(
                (p: any) => p.email || p.displayName || p,
              ),
              meet_link:
                pending.hangout_link || pending.meet_link || 'https://meet.google.com/abc-defg-hij',
            })
          }
        }
      }
  ...
  const send = async (mode: 'voice' | 'text') => {
    ...
    try {
      ...
      if (previewId && meetingStart && meetingEnd) {
        const attendeeEmails = (mData?.attendees || []).map((a: any) =>
          typeof a === 'object' ? a.email || a.name : String(a),
        )
        setActiveProposal({
          preview_id: previewId,
        ...
        try {
          const newMeetingObj = {
            ...
            proposed_slots: [
              { start: meetingStart, end: meetingEnd, title: meetingTitle, meet_link: meetLink },
            ],
            created_at: new Date().toISOString(),
  ...
  const summarizeForSpeech = (fullText: string, spokenText: string): string => {
    ...
    if (
      ...
      fullText.toLowerCase().includes('calendar') ||
      fullText.toLowerCase().includes('scheduled')
    ) {
          ...
          <div className="max-h-[420px] space-y-4 overflow-y-auto pr-2">
            {transcript.map((m) => (
              ...
              >
                ...
                {(m.draftId ||
                  m.agentUsed === 'reply_agent' ||
                  m.content.includes('Reply Draft') ||
                    ...
                    <div className="mt-3 rounded-lg border border-primary/40 bg-background p-3.5 space-y-2.5 shadow-sm text-xs">
                      ...
                      <div className="flex flex-wrap items-center gap-2 pt-1">
                        <Link href="/replies">
                          <Button
                            size="sm"
                            className="bg-primary text-primary-foreground hover:bg-primary/90 font-semibold text-xs gap-1.5"
                          >
                            Open Reply Drafts Page (/replies){' '}
                        ...
                        {m.draftId && (
                          <Button
                            ...
                            variant="secondary"
                            onClick={() =>
                              handleInstantApproveAndSend(m.draftId!, m.draftBody || '')
                            }
                ...
                {(m.agentUsed === 'calendar_agent' ||
                  m.content.includes('Meeting scheduled successfully') ||
                  m.content.includes('meeting') ||
                  m.content.includes('Calendar Proposal')) &&
                  m.role === 'assistant' && (
                    <div className="mt-3 rounded-lg border border-emerald-500/40 bg-emerald-500/5 p-3.5 space-y-2.5 shadow-sm text-xs">
                      ...
                      {activeProposal?.source_email && (
                        <div className="rounded bg-emerald-500/10 p-2.5 space-y-1 border border-emerald-500/20 text-[11px]">
                          <div className="flex items-center justify-between">
                            ...
                            <Link href="/inbox">
                              <Button
                                size="sm"
                                variant="ghost"
                                className="h-5 text-[10px] px-1.5 text-emerald-600 dark:text-emerald-400 hover:underline"
                              >
                                Open Original Email ➔
                              </Button>
                            </Link>
                          ...
                          <div>
                            ...
                            {activeProposal.source_email.subject}"
                          </div>
                          {activeProposal.source_email.from && (
                            <div>
                              <span className="font-semibold">From:</span>{' '}
                              {activeProposal.source_email.from.name} &lt;
                              {activeProposal.source_email.from.email}&gt;
                            </div>
                          )}
                        </div>
                      )}
                      ...
                      <div className="flex flex-wrap items-center gap-2 pt-1">
                        <Link href="/calendar">
                          <Button
                            size="sm"
                            className="bg-emerald-600 text-white hover:bg-emerald-700 font-semibold text-xs gap-1.5"
                          >
                            View & Manage in Calendar Page (/calendar){' '}
                            <ChevronRight className="h-3.5 w-3.5" />
                          </Button>
                        </Link>
                        <Link href="/inbox">
                          <Button
                            size="sm"
                            variant="outline"
                            className="border-emerald-500/40 text-emerald-600 dark:text-emerald-400 font-semibold text-xs gap-1.5"
                          >
      ...
      <div className="space-y-4">
        ...
        {activeProposal && (
          <Card className="p-4 border-emerald-500/60 bg-card shadow-xl space-y-3">
              ...
              <div className="rounded-md bg-emerald-500/15 border border-emerald-500/30 p-3 text-emerald-800 dark:text-emerald-200 font-sans space-y-1.5 shadow-sm">
                <div className="flex items-center justify-between">
                  ...
                  <Link href="/inbox">
                    <Button
                      size="sm"
                      variant="ghost"
                      className="h-5 text-[10px] px-2 text-emerald-600 dark:text-emerald-300 hover:underline font-medium"
                    >
                      Open Original Email ➔
                    </Button>
                  </Link>
                ...
                {(activeProposal.source_email?.from?.email ||
                  ...
                  <div className="text-[11px] opacity-90">
                    <span className="font-semibold">From / Sender:</span>{' '}
                    {activeProposal.source_email?.from?.name || activeProposal.target_email?.sender}{' '}
                    {activeProposal.source_email?.from?.email &&
                    activeProposal.source_email.from.email !== activeProposal.source_email.from.name
                      ? `<${activeProposal.source_email.from.email}>`
                      : ''}
                  </div>
                )}
                {activeProposal.attendees && activeProposal.attendees.length > 0 && (
                  <div className="text-[11px] opacity-90 truncate">
                    <span className="font-semibold">To / Receiver:</span>{' '}
                    {activeProposal.attendees.join(', ')}
                  </div>
                )}
                {activeProposal.source_email?.summary && (
                  <div className="text-[10px] opacity-85 italic line-clamp-3 pt-1 border-t border-emerald-500/20">
                    <span className="font-semibold not-italic">Email Brief:</span> "
                    {activeProposal.source_email.summary}"
                  </div>
                )}
              </div>
            ...
            <div className="space-y-1.5 text-xs">
              ...
              <div>
                ...
                <span className="font-medium text-foreground">
                  {new Date(activeProposal.start).toLocaleString([], {
                    dateStyle: 'medium',
                    timeStyle: 'short',
                  })}{' '}
                  –{' '}
                  {new Date(activeProposal.end).toLocaleTimeString([], {
                    hour: '2-digit',
                    minute: '2-digit',
                  })}
                </span>
            ...
            {activeProposal.double_booking_warnings &&
              activeProposal.double_booking_warnings.length > 0 && (
                <div className="rounded-md border border-amber-500/40 bg-amber-500/10 p-2.5 text-xs text-amber-700 dark:text-amber-300 space-y-1">
                  ...
                  <p className="text-[11px] opacity-90">
                    {activeProposal.double_booking_warnings.length} pending proposal(s) overlap with
                    this candidate time window!
                  </p>
                </div>
              )}
            ...
            <div className="flex flex-wrap items-center gap-2 pt-1">
              <Button
                ...
                onClick={() =>
                  handleInstantConfirmCalendarEvent(
                    activeProposal.preview_id,
                    activeProposal.approval_id,
                  )
                }
              >
              ...
              <Link href="/calendar">
                <Button
                  size="sm"
                  variant="outline"
                  className="text-xs gap-1 border-emerald-500/40 text-emerald-600 dark:text-emerald-400"
                >
          ...
          <Card className="p-4 border-primary/60 bg-card shadow-xl space-y-3">
            ...
            <div className="space-y-1 text-xs">
              <div>
                ...
                <span className="font-medium text-foreground">
                  {activeDraft.recipient || 'Recipient'}
                </span>
              </div>
              <div>
                <span className="font-semibold text-muted-foreground">Subject:</span>{' '}
                <span className="font-medium text-foreground">
                  {activeDraft.subject || 'Reply Draft'}
                </span>
              </div>
            </div>
            ...
            {activeDraft.has_gaps && (
              <div className="rounded-md border border-amber-500/40 bg-amber-500/10 p-2.5 text-xs text-amber-700 dark:text-amber-300 space-y-1">
                ...
                {activeDraft.gap_notes && activeDraft.gap_notes.length > 0 ? (
                  ...
                  <p className="text-[11px] opacity-90">
                    Missing unverified facts flagged in draft text.
                  </p>
                )}
              </div>
            )}
            ...
            <div className="flex flex-wrap items-center gap-2 pt-1">
              <Button
                ...
                onClick={() =>
                  handleInstantApproveAndSend(activeDraft.draft_id, activeDraft.draft_body)
                }
              >
          ...
          <Card className="p-4 border-primary/40 shadow-lg">
              ...
              {queryResults.map((item, idx) => (
                ...
                >
                  <div className="flex items-center justify-between gap-1 mb-1">
                    <span className="font-semibold truncate text-foreground">{item.sender}</span>
            ...
            {selectedEmail && (
              <div className="mt-3 p-3 bg-muted/40 rounded-lg border text-xs space-y-1">
                ...
                <div>
                  <span className="text-muted-foreground">From:</span> {selectedEmail.sender}
                </div>
                <div>
                  <span className="text-muted-foreground">Subject:</span> {selectedEmail.subject}
                </div>
            ...
            <>
              <Card className="p-4">
                ...
                <p className="mt-2 text-xs text-muted-foreground">
                  When you issue an email query command or request a reply draft, the results and
                  interactive AI draft card will render right here in this side window!
                </p>
              </Card>
